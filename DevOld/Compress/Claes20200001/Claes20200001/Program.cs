...
namespace Charlotte
{
	class Program
	{
		...
		private void Compress(string rDir, string wFile)
		{
			...
			using (GZipStream gz = new GZipStream(fileWriter, CompressionMode.Compress)) // BUG: 第3引数にtrueが要る -- 顕在化しないBug @ 2023.5.4
			{
				this.Writer = gz;
		...
		private void Decompress(string rFile, string wDir)
		{
			...
			using (GZipStream gz = new GZipStream(fileReader, CompressionMode.Decompress)) // BUG: 第3引数にtrueが要る -- 顕在化しないBug @ 2023.5.4
			{
				this.Reader = gz;
